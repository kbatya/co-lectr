...
The lecturer talks to it directly through `adk web`, and it carries real tools: `list_submissions`, `run_checks` (the layer-1 facts for one student), `read_file`, `class_digest` (recomputed live from the checkout), `stored_class_digest` (what has accumulated in Firestore this milestone), and `flagged_reviews` (*did anyone try to game the reviewer?*). It refuses to review anyone until it's been told which chapters were taught — flagging a chapter-5 concept during chapter 2 is the fastest way to lose a student's trust, so it asks first.
...
- **Firestore** holds three collections — `reviews/{repo}#{pr}#{sha}`, `students/{login}`, and `classes/{class}/milestones/{milestone}` — with atomic `Increment` / `ArrayUnion` counters so two concurrent reviews in the same class don't race.
...
- **118 tests, and they run offline.** The network and the model are faked, so the suite needs no credentials; the Firestore integration tests skip without them, so a clean-machine run is **110 passed, 8 skipped**. They cover signature verification, routing, the review core, output validation, aggregation, the reserve/idempotency lock, and the fetch/post client — and include an `InMemoryRunner`-driven round-trip of both a normal review and an injection attempt.
